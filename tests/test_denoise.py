...
def test_denoise_series_shapes():
    ...
    res = denoise_series(x, window=100, overlap=0.5, lam_var=1.0, max_iters=50, n_jobs=1)
...
def test_variance_non_increase():
    ...
    res = denoise_series(x, window=200, overlap=0.5, lam_var=1.0, max_iters=50)
    assert np.var(np.diff(res.denoised)) <= np.var(np.diff(x)) + 1e-9

def test_tiny_input():
    x = np.array([100.0, 100.1, 99.9, 100.0])
    res = denoise_series(x, window=8, overlap=0.5, lam_var=0.5, max_iters=10)
    assert res.denoised.shape == x.shape
